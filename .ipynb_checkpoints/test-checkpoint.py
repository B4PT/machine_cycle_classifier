...
def test_binarize_targets(sample_data):
    ...
    assert all(y_binarized == label_binarize(y, classes=[0, 1]).flatten()), "Target binarization is incorrect"
...
def test_train_test_split(sample_data):
    ...
    assert X_test.shape[0] == sample_data.shape[0] - TRAIN_SIZE, "Number of test samples is incorrect"
